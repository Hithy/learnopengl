...
GLBlending::GLBlending() : GLBase("hello_gl", 1920, 1080) {}
...
void GLBlending::Init() {
  _shader = new Shader("shader/depth.vert", "shader/depth.frag");
  // _grass_shader = new Shader("shader/depth.vert", "shader/depth.frag");

  // gen texture
  stbi_set_flip_vertically_on_load(true);
  _cube_texture = GenTexture("asserts/images/marble.jpg");
  _floor_texture = GenTexture("asserts/images/metal.png");
  _grass_texture = GenTexture("asserts/images/grass.png");
  _window_texture = GenTexture("asserts/images/blending_transparent_window.png");
  ...
  _shader->use();
  _shader->SetUniform1i("texture1", 0);
...
void GLBlending::Draw() {
  ...
  std::map<float, glm::vec3> distance_map;
  for (const auto &vege_pos : vegetation) {
    float dis = glm::length(vege_pos - _camera.pos);
    distance_map[dis] = vege_pos;
  }

  glBindTexture(GL_TEXTURE_2D, _window_texture);
  for (auto itr = distance_map.rbegin(); itr != distance_map.rend(); itr++) {
    model = glm::translate(glm::mat4(1.0f), itr->second);
    _shader->SetUniformMat4fv("model", glm::value_ptr(model));
